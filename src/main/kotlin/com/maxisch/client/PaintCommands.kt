        ...
        ClientCommandRegistrationCallback.EVENT.register { dispatcher, _ ->
            dispatcher.register(
                ClientCommands.literal("ap")
                    .executes { context ->
                        // Deferred: the chat screen closes itself right after this command runs,
                        // which would immediately null out the screen we just opened.
                        Minecraft.getInstance().execute {
                            Minecraft.getInstance().setScreenAndShow(PainterScreen.open())
                        }
                        1
                    }
                    .then(
